...
def inspect_text(text: str) -> InspectionResult:
    ...
    return InspectionResult(
        ...
        action=action,
        masked_text=masked if action in {PolicyAction.MASK, PolicyAction.ALLOW} else None,
        reasons=reasons,
    )


def guard(user_id: str, prompt: str, output: str | None = None) -> GuardResponse:
    _ = user_id  # reserved for risk profiling (M4)
    ...
    elif output_result and output_result.action == PolicyAction.MASK and final_action == PolicyAction.ALLOW:
